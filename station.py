...
class Station(object):
    def __init__(self, idx: int, observation_area: StationObservationArea, database):
        ...
        self.database.add_station.remote(
            self.idx, self.observation_area.x_min, self.observation_area.x_max,
    ...
    def read_report(self) -> None:
        ...
        if len(activity.getAttribute('x')) > 0:
            self.database.add_observation.remote(
                self.idx, int(activity.getAttribute('x')),
